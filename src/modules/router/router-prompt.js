...
export function buildRouterGuidelines({ format = 'markdown' } = {}) {
  if (format === 'compact') {
    ...
    text += `- ถ้า contract เป็น \`GENERAL\` แปลว่าไม่มีขั้นตอนเฉพาะของ STeP สำหรับงานนี้ ให้ช่วยทันทีเหมือนผู้ช่วยทั่วไปโดยไม่ถามกลับเพื่อเลือกประเภทงาน ถามได้เฉพาะเมื่อขาดเนื้อหาที่ต้องใช้จริง (เช่น ข้อความที่จะแปล) ใช้กฎใน \`mandatoryReferences\` และห้ามอ้างว่าเป็นระเบียบหรือแบบฟอร์มของ STeP ถ้าไม่มีเอกสารอ้างอิง\n`;
    text += `- ถ้า contract เป็น \`CLARIFY\` ให้ถาม \`clarification.question\` ทีละ 1 ข้อเพื่อแยกประเภทงาน โดยใช้บริบทที่รู้แล้วและไม่ให้ผู้ใช้เลือกชื่อ Skill; รอคำตอบก่อนโหลด Skill\n`;
    text += `- ถ้ามี \`clarification.options\` ให้แสดง label ตามลำดับพร้อมหมายเลขเดิม เพื่อแยกงานหลายขั้นที่ก้ำกึ่งกัน; รับข้อความหรือหมายเลขได้ เก็บคำตอบสะสมแยกบรรทัด โดยคำตอบล่าสุดอยู่บรรทัดสุดท้าย\n`;
    text += `- เมื่อได้คำตอบ ให้คงคำขอเดิมและคำตอบก่อนหน้า แล้ว route ใหม่ด้วย \`step-ai ask "<คำขอเดิม>" --answer "<คำตอบสะสม>" --json\`; ถ้ายัง CLARIFY ให้ถามเจาะจงเฉพาะสิ่งที่ยังขาด ห้ามถามข้อมูลเดิมซ้ำ เมื่อชัดแล้วให้ทำงานต่อ\n`;
    text += `- **ชวนตรวจก่อนนำไปใช้:** หลังคำตอบที่ผู้ใช้จะนำไปตัดสินใจหรือใช้จริง (ตัวเลขประมาณการ คำแนะนำ แผน ร่างข้อเสนอ การตีความข้อมูล) ให้ต่อท้ายว่า "ลองเช็กเพิ่มก่อนใช้:" ตามด้วยคำถาม 2–3 ข้อที่ผู้ใช้ถามกลับได้ทันที แต่ละข้อชี้สิ่งเฉพาะในคำตอบ ทำครั้งเดียวต่อบทสนทนา และข้ามเมื่อเป็นงานสรุปหรือจัดรูปแบบจากข้อมูลของผู้ใช้เอง งานสร้างสรรค์ การถามทั่วไป หรือผู้ใช้ขอให้ตรวจหรือขอแบบสั้นอยู่แล้ว\n`;
  ...
  text += `- **คะแนน $< 0.50$ (FALLBACK Tier)**: ถ้าคำขอบอกงานมาชัด contract เป็น \`GENERAL\` ให้ช่วยทันทีในฐานะผู้ช่วยทั่วไปภายใต้กฎองค์กร ถ้าคำขอไม่บอกว่าให้ทำอะไร (เช่น "ช่วยหน่อย") contract เป็น \`CLARIFY\` ให้ถามหนึ่งคำถาม\n\n`;
  ...
  text += `- **ถามทีละ 1 ข้อเพื่อระบุขอบเขต**: ใช้ cheap context และคำตอบก่อนหน้าก่อนถาม เลือกถามสิ่งที่ช่วยแยกประเภทงานมากที่สุด เช่น เอกสารใช้ทำเรื่องอะไร หรืออยากได้ผลลัพธ์อะไร ห้ามถามสิ่งที่รู้แล้วซ้ำ\n`;
  text += `- **เสนอ 2–3 ทางเลือกเมื่อช่วยให้ตอบง่าย**: ใช้ภาษางานทั่วไป เปิดให้ตอบนอกตัวเลือกได้ และไม่ให้พนักงานเลือกชื่อ Skill\n`;
  text += `- ถ้ามี \`clarification.options\` แสดง label และหมายเลขตามลำดับเดิม เพื่อแยกระหว่าง Playbook ที่ก้ำกึ่ง รับข้อความหรือหมายเลขผ่าน --answer; คำตอบสะสมแยกบรรทัดและคำตอบล่าสุดอยู่บรรทัดสุดท้าย โดยยังเก็บคำขอเดิมไว้ตรวจ Authority\n`;
  text += `- เมื่อ local contract เป็น \`GENERAL\` ห้ามถามเพื่อเลือกประเภทงาน ให้ช่วยงานทันที ถามได้เฉพาะเนื้อหาที่ขาดจริง และไม่อ้างระเบียบ STeP ที่ไม่มีแหล่งอ้างอิง\n`;
  text += `- ถ้าเรียก \`step-ai\` ไม่ได้ ให้ใช้ \`sh ./step-ai\` (macOS/Linux) หรือ \`.\\step-ai.cmd\` (Windows) จากโฟลเดอร์ workspace\n`;
  text += `- เมื่อ local contract เป็น \`CLARIFY\` ให้ใช้ \`clarification.question\` เป็นคำถามตั้งต้น รอคำตอบก่อนโหลด Skill แล้วเรียก \`step-ai ask "<คำขอเดิม>" --answer "<คำตอบสะสม>" --json\` ใหม่ หยุดถามเมื่อได้เส้นทางชัดเจน; Authority BLOCK และประตูยืนยันยังมีลำดับก่อนการถามขอบเขต\n`;
  text += `- **ชวนตรวจก่อนนำไปใช้ (ครั้งเดียวต่อบทสนทนา)**: หลังคำตอบที่ผู้ใช้จะนำไปตัดสินใจหรือใช้จริง ให้ตอบให้ครบก่อน แล้วต่อท้ายด้วย "ลองเช็กเพิ่มก่อนใช้:" และคำถาม 2–3 ข้อ แต่ละข้อชี้ตัวเลข ขั้นเหตุผล หรือสมมติฐานเฉพาะในคำตอบ เขียนแบบที่ผู้ใช้ส่งกลับมาถามได้ทันที ไม่เกินข้อละหนึ่งบรรทัด ข้ามเมื่อเป็นการสรุปหรือจัดรูปแบบข้อมูลที่ผู้ใช้ให้มา งานสร้างสรรค์ คำถามทั่วไป หรือเมื่อผู้ใช้ขอให้ตรวจ อ้างอิง หรือขอแบบสั้นอยู่แล้ว\n`;
